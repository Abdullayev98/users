                ...
                <div class="msg {{ $message->user_id == Auth::user()->id ? 'right-msg' : 'left-msg' }} ">
                    ...
                    <div class="msg-bubble">
                        ...
                        <div class="msg-text">
                            {{ $message->text }}
                            <div>
{{--                                @foreach(json_decode($message->users) as $img)--}}
...
{{--                                @endforeach--}}
                ...
                @foreach ($conversations as $conversation)
                    ...
                    @php
                        $message_user = \App\Models\User::where('id', $conversation->user_id)->first();
                    ...
                    <div class="msg {{ $conversation->user_id == Auth::user()->id ? 'right-msg' : 'left-msg' }}">
                        ...
                        <div class="msg-bubble">
                            <div class="msg-info">
                                <div class="msg-info-name">{{ $message_user->name }}</div>
            ...
            @if($message->status == 1 || $message->status == 2)
                <form action="{{ route('conversation.send', $message->id) }}" method="post" class="msger-inputarea">
                    @csrf
            ...
            @else
                <form action="{{ route('conversation.send', ['message'=>$message->id]) }}" method="post" class="msger-inputarea">
                    @csrf
                ...
                @elseif(($message->status==1 || $message->status==2 ) && !Auth::user()->hasRole('user'))
                    <form action="{{ route('appeal.close', $message) }}" method="POST">
                        ...
                        <button onclick="this.form.submit()" type="button"
                                class="btn btn-danger">@lang('site.close')</button>
                    </form>
                @else
                    <button type="button" class="btn disabled buttonDis">@lang('site.close')</button>
                @endif
    ...
    <form id="submit" action="{{route('conversation.rating',$message)}}" method="POST">
        ...
        <div class="stars">
            <input type="radio" id="star1" name="rating" value="1"/><input type="radio" id="star2" name="rating"
                                                                           value="2"/><input type="radio" id="star3"
                                                                                             name="rating"
                                                                                             value="3"/><input
                type="radio" id="star4"
