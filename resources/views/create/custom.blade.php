                        ...
                        <div class="py-4 mx-auto  text-left ">
                            <div class="mb-4">
                                ...
                                {{ $data->getTranslatedAttribute('label',Session::get('lang') , 'fallbackLocale') }}
                                    <select id="where" name="{{$data->name}}" class="shadow appearance-none border focus:shadow-orange-500 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none"  required>

                                @foreach($data->options['options'] as $key => $option)
                                        <option @if($key == $data->values) selected @endif value="{{$option}}">{{$option}}</option>
                                @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                @foreach($data->options['options'] as $key => $option)
                                        <label class="md:w-2/3 block mt-6">
                                                    <input @if($key == $data->values) checked @endif class="mr-2  h-4 w-4" type="checkbox" value="{{$option}}" name="{{$data->name}}">
                            ...
                            <div class="mb-4">
                                <div id="formulario" class="flex flex-col gap-y-4">
                                    ...
                                    <div>
                                        ...
                                        <input @if($key == $data->values) checked @endif type="radio" id="{{$key}}" name="{{$data->name}}" value="{{$option}}">
...
@if($data->type == 'input')
                        ...
                        {{ $data->getTranslatedAttribute('label',Session::get('lang') , 'fallbackLocale') }}
                                        <input placeholder="{{ $data->getTranslatedAttribute('placeholder',Session::get('lang') , 'fallbackLocale') }}"
                                         id="car" name="{{$data->name}}" type="text" class="shadow appearance-none border focus:shadow-orange-500 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none"  required>
