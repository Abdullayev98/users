...
class PaycomTransaction extends Database
{
    ...
    public function save()
    {
        ...
        if (!$this->id) {
            ...
            $sql               = "INSERT INTO paycom_transactions SET 
            ...
            $is_success = $sth->execute([
                ...
                ':pTransactionId' => 1 * $this->transaction_id,
            ]);
            ...
            $sql = "UPDATE paycom_transactions SET
    ...
    public function find($params)
    {
        ...
        if (isset($params['id'])) {
            $sql        = "SELECT * FROM paycom_transactions WHERE paycom_transaction_id = :pPaycomTxId";
            $sth        = $db->prepare($sql);
            $is_success = $sth->execute([':pPaycomTxId' => $params['id']]);
        } elseif (isset($params['account'], $params['account']['transaction_id'])) {
            ...
            $sql        = "SELECT * FROM paycom_transactions WHERE state IN (1, 2) AND transaction_id = :pTransactionId";
            $sth        = $db->prepare($sql);
            $is_success = $sth->execute([':pTransactionId' => $params['account']['transaction_id']]);
        } else {
        ...
        if ($is_success) {
            ...
            if ($row) {
                ...
                $this->transaction_id        = 1 * $row['transaction_id'];
                return $this;
            }
    ...
    public function report($from_date, $to_date)
    {
        ...
        foreach ($rows as $row) {
            $result[] = [
                ...
                'amount'       => 1 * $row['amount'],
                'account'      => [
                    'transaction_id' => 1 * $row['transaction_id'], // account parameters to identify client/order/service
